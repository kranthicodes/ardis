...
async function ensureArconnectFeeTxTableExists(tableName: string) {
...
class Indexer extends BaseIndexer {
  tableName: string = "ardis_arconnect_fee_tx";
  constructor(name: string) {
    super(name);
  }

  async traverseAndStore() {
    await ensureArconnectFeeTxTableExists(this.tableName);
...
const indexer = new Indexer("arconnect_fee_tx");
