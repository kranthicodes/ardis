...
async function ensureArconnectSignedTxTableExists(tableName: string) {
...
class Indexer extends BaseIndexer {
  tableName: string = "ardis_arconnect_signed_tx";
  constructor(name: string) {
    super(name);
  }

  async traverseAndStore() {
    await ensureArconnectSignedTxTableExists(this.tableName);
...
const indexer = new Indexer("arconnect_signed_tx");
